      ...
      <div className="flex gap-14 flex-wrap mt-12 justify-center">
        {blogs ? blogs.map(({ title, description, url }, index) => (
          <Link href={url || ""} key={index}>
            <span className="flex-col flex-1 h-40 text-start backdrop-blur-sm p-4 gap-2 font-semibold border flex border-slate-200 rounded-xl w-full max-w-[380px] hover:border-blue-1 shadow hover:shadow-md hover:scale-[1.02] transition-all">
