...
export default function QuizCompo({ questions: questionsProp, category, blog }: QuizCompoProps) {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post('/api/blogSystem/validate-answers', {
        category,
        blog,
        answers: questions ? questions.map(({ question, userAnswer }) => ({ question, answer: userAnswer })) : [],
      });

      const validationResults = response.data;
      setQuestions(validationResults ? validationResults.map(({ correct, rightAnswer }: ValidateResponse, index: number) => ({
        ...questions[index],
        correct: correct,
  ...
  return (
    <div className="p-2">
      {questions ? questions.map(({ question, choices, correct, userAnswer, correctAnswer }, qIndex) => (
