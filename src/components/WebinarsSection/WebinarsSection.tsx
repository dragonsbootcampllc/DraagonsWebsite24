...
export default function WebinarsSection({handel_popup}: WebinarsSectionProps) {
    const videoSrc = '/Video/Webinar_Video.mp4';
    const webinarTitle = 'Webinar Name';
    const webinarDescription = 'Lorem ipsum dolor sit amet consectetur adipisicing elit. eos velit expedita aut quasi minus molestias!';
    ...
    return (
        <div className="relative w-full flex flex-col gap-12 justify-center items-center">
            <div className='w-full flex justify-center flex-col items-center'>
                <h1 className="text-7xl font-bold blur-sm text-white/30 animate-pulse">Ready For a Sprint?</h1>
                <h1 className="text-7xl font-bold text-white/80 -mt-16">Ready For a Sprint?</h1>
            </div>
            <div className="relative w-full group max-w-5xl overflow-hidden rounded-lg ">
                ...
                <button onClick={onOverlayClick} className='w-16 aspect-square transition-all hover:animate-pulse rounded-full justify-center items-center flex absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 bg-white/50 opacity-50 group-hover:opacity-100 shadow-white'><Play stroke='black' size={32} strokeWidth={3} /></button>
            </div>
        </div>
    );
};
