...
const Dropdown_V2 = ({ isTabOpen, toggleTab, closeTab, data }) => {
  ...
  return (
    <div
      className={
        "group min-[1096px]:h-full cursor-pointer max-[1096px]:w-full text-white "
      }
    >
      <div
      ...
      >
        {/* Start Button */}
        <span
          className={`px-4 font-semibold flex items-center justify-center min-[1096px]:h-full transition-all duration-[.45s] max-[1096px]:py-8 max-[1096px]:hover:bg-transparent max-[1096px]:px-0 relative
        ...
        {/* Start Dropdown */}
        <div
          className={`w-screen transition-all flex justify-center items-center min-[1096px]:absolute min-[1096px]:left-1/2 min-[1096px]:top-[calc(100%+1px)] box-border min-[1096px]:-translate-x-1/2 min-[1096px]:shadow-md border-b border-b-purple-900 bg-[#0d0124] h-0 ${isTabOpen && "max-[1096px]:h-auto max-[1096px]:py-5"} overflow-hidden min-[1190px]:group-hover:h-[622px] min-[1096px]:group-hover:h-[640px]`}
        >
          {/* Start Inner Container */}
          <div className="max-w-[1400px] text-start gap-5 w-full justify-start flex items-start pt-10 transition-all px-4 box-border max-[1096px]:flex-col h-full">
            <div className="w-1/2 max-[1096px]:w-full flex flex-col gap-4">
              {/* Start Tab One */}
              <Link
                href={data.tab1.path || ""}
                className="active:scale-90 w-full min-h-[240px] group/learn max-[1096px]:w-[90%] max-[1096px]:mx-[5%] box-border hover:shadow-md hover:border-2 transition-all px-12 flex justify-between items-center bg-purple-900/40 border border-purple-900 rounded-xl gap-2"
                onClick={closeTab}
              >
                <span className="flex flex-col items-start gap-2 pt-4">
                  ...
                  <span className="max-[1096px]:mb-5 flex justify-center items-center group-hover/learn:ml-5 text-white/70 group-hover/learn:text-purple-400 transition-all font-light mt-2">
                    Learn More{" "}
            ...
            <div className="flex flex-col w-1/2 gap-4 max-[1096px]:w-full">
              {/* Start Tab Two */}
              <Link
                href={data.tab2.path || ""}
                className="active:scale-90 min-h-[240px] w-full group/learn max-[1096px]:w-[90%] border-dashed hover:border-solid max-[1096px]:mx-[5%] box-border hover:shadow-md px-12 flex group/learn justify-center items-center rounded-xl gap-2 transition-all border border-purple-600 hover:border-purple-900 hover:border-2"
                onClick={closeTab}
              >
                <span className="flex flex-col items-start gap-2 pt-4 ">
                  <span className="font-bold">{data.tab2.title}</span>
                  <span className="text-white/80">{data.tab2.description}</span>
                  <span className="max-[1096px]:mb-5 flex justify-center items-center group-hover/learn:ml-5 group-hover/learn:text-purple-400 transition-all font-light mt-2">
                    Learn More{" "}
                    <span className="text-lg box-border pt-1">
                      <MdArrowRight />
                    </span>
