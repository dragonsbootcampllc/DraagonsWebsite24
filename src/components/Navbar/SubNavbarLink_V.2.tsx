...
export default function SubNavbarLink_V2({ Icon, title, path, clear, color }: SubNavbarLinkProps) {
  ...
  return (
    <Link href={path} className={`group/subLink flex justify-start items-center gap-2 p-3 pl-0 text-sm`} onClick={clear}>
      ...
      <span className={`text-sm text-white/70 group-hover/subLink:text-[#efefef]`}>{title}</span>

      {/* To generate classes in tailwindCSS */}
      <input type="hidden" className='bg-[#8e44ad] text-[#8e44ad]' />
    </Link>
  );
}
