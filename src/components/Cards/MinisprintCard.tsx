...
export default function MovieCard(props: { movie: any; exploreUrl: string; expandImage: any; }) {
  ...
  return (
    <div
      className={`relative transition-all duration-300 ease-in-out ${isHovered ? "md:aspect-video  md:h-96" : "aspect-[2/3] h-96"}`}
    ...
    >
      <div className='w-full h-full relative rounded-3xl overflow-hidden'>
        ...
        {isHovered && (
          <div className='absolute inset-0 shadow-inner bg-black/10 flex flex-col justify-end p-4'>
            <div className="absolute inset-0 flex flex-col justify-end p-4 max-w-96">
              ...
              <p className='text-gray-300 text-xs mb-4 line-clamp-3'>{movie.description}</p>
        ...
        <div className='md:hidden flex flex-col justify-end p-4 absolute inset-0 bg-black/20'>
          <h2 className='text-white text-lg font-bold mb-2'>{movie.title}</h2>
          <p className='text-gray-300 text-sm mb-2'>{movie.duration}</p>
          <p className='text-gray-300 text-xs mb-4 line-clamp-3'>{movie.description}</p>
          <Link href={`${currentURL}/${exploreUrl}`} passHref>
