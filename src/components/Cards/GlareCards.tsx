...
import { EssentialSkillscontent,essentialSkills, GlareCardContent } from "./../JopReady/CTAMaterial/data"; 
const GlareCards: React.FC = () => {
  const {
    headlineText,
    heroHighlightedText,
    descriptionText,
          ...
          {glareCards.map((card: GlareCardContent, index: number) => (
            <GlareCard
              ...
              {card.title && card.description && (
                <span className='absolute left-0 right-0 grid place-items-center p-4 text-center h-flex bg-purple-900/80 backdrop-blur-3xl  w-full bottom-0 rounded-3xl  '>
