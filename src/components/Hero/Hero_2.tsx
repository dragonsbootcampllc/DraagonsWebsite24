...
export default function Hero_2({ News, sub_heading_text, hero_highlighted_text, hero_heading, description }: HeroProps) {
  return (
    <div className="min-h-[700px] md:min-h-[calc(100vh-12rem)] relative">
      <div className="mx-auto pt-[120px] md:pt-[140px] flex flex-col justify-between gap-9 items-center relative z-10 px-4">
        <div className="content flex flex-col gap-12 text-center">
          <div className="mb-6 p-2 flex justify-center">
            <Link href={News.link} passHref className="flex items-center active:scale-95 duration-200 gap-1 text-md bg-purple-400/10 shadow-gray-200/20 shadow-inner px-2 rounded-full min-w-[30px]">
          ...
          <div className="text-4xl md:text-5xl max-w-4xl lg:text-6xl xl:text-7xl text-gray-200 font-semibold">
            <h1>
              {hero_heading}
              {sub_heading_text}
          ...
          <p className="text-white/50 text-base md:text-lg mx-auto flex flex-col gap-6 justify-center max-w-2xl">
            {description}
          </p>
        </div>
        <div className="text-center  mt-8 flex flex-col gap-4 sm:flex-row sm:gap-8 justify-center">
